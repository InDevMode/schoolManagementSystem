...
@section('content')
    <div class="m-5">
        ...
        @include('message')
        <div class="my-5">
            {{ $getMyAttendance->links('vendor.pagination.tailwind') }}
        </div>
        <div
                    ...
                    <div class="w-full">
                        <div class="relative">
                            <input id="attendance_date" name="attendance_date" value="{{ Request::get('attendance_date') }}"
                                ...
                                placeholder="date de présence..." data-class="flatpickr-right" required />
        ...
        <div class="mt-5">
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @if(!empty($getMyAttendance) && $getMyAttendance->count() > 0)
                            @foreach($getMyAttendance as $myAttendance)
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            <td class="px-6 py-4">
                                                {{ $myAttendance->class_name }}
                                            </td>
                                            <td class="px-6 py-4">
                                                <span
                                                    class="
                                ...
                                ($myAttendance->attendance_type == 2 ? 'Retard' :
                                    ($myAttendance->attendance_type == 3 ? 'Absent(e)' :
                                        ($myAttendance->attendance_type == 4 ? 'Demi-journée' : 'Non défini'))) }}
                                            ...
                                            <td class="px-6 py-4">
                                                {{ \Carbon\Carbon::parse($myAttendance->attendance_date)->locale('fr')->translatedFormat('d M Y') }}
                                            </td>
                                            <td class="px-6 py-4">
                                                {{ \Carbon\Carbon::parse($myAttendance->created_at)->locale('fr')->translatedFormat('d M Y H:i:s') }}
                                            </td>
                                        </tr>
                            @endforeach
                            @if(empty($getMyAttendance) && !$getMyAttendance->isEmpty())
