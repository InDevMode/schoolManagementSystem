...
<form :action="editing ? '{{ url('chat.update') }}/' + editId : '{{ route('chat.send') }}'" method="POST"
      enctype="multipart/form-data" x-data="{ fileName: '', filePreview: '', fileType: '', showEmoji: false }">
      {{ csrf_field() }}
      ...
      <div class="bg-white dark:bg-black border-t border-gray-300 dark:border-black w-full">
            <div class="flex items-center py-2 px-3 bg-gray-50 rounded-lg dark:bg-gray-700 relative">
                  ...
                  <div x-show="showEmoji" @click.outside="showEmoji = false"
                        class="absolute bottom-14 left-0 z-50 p-2 w-72">
                        <emoji-picker @emoji-click="(e) => $refs.message.value += e.detail.unicode"></emoji-picker>
                  </div>

                  <!-- 💬 Zone de texte -->
                  <textarea id="message" rows="1" name="message" x-ref="message" x x-model="$root.editMessage" required
                  ...
                  <input type="hidden" name="id" :value="$root.editId">
                  ...
                  <button type="submit"
                        ...
                        <iconify-icon :icon="editing ? 'mdi:check-bold' : 'mdi:send'" width="24"
                              height="24"></iconify-icon>
                  </button>

                  <!-- ❌ Bouton annuler édition -->
                  <button type="button" x-show="$root.editing"
                        @click="$root.editing=false; $root.editMessage=''; $root.editId=null"
                        class="ml-2 p-2 text-red-500 hover:bg-red-100 dark:hover:bg-gray-600 rounded-full">
