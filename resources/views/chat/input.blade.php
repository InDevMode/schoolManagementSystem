...
<form :action="editing ? '{{ url('chat') }}/' + editId : '{{ route('chat.send') }}'" method="POST"
      enctype="multipart/form-data" x-data="{ fileName: '', filePreview: '', fileType: '', showEmoji: false }" x-ref="chatForm"
      @submit="
          if (editing) {
              setTimeout(() => {
                  editing = false;
      ...
      <div class="bg-white dark:bg-black border-t border-gray-300 dark:border-black w-full">
            <div class="flex items-center py-2 px-3 bg-gray-50 rounded-lg dark:bg-gray-700 relative">
                  ...
                  <textarea id="message" rows="1" name="message" x-ref="message" x-model="editMessage" required
