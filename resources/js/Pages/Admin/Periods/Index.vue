<template>
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Périodes</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{{ periods.total }} période(s) enregistrée(s)</p>
            </div>
        ...
        <AppModal v-model="showDelete" title="Supprimer la période" size="sm" persistent>
...
const form = useForm({ name: '', start_date: '', end_date: '', status: '1', settings_id: '1' });
...
const submitForm = () => {
    if (editTarget.value) {
        form.post(`/admin/examinations/period/edit/${editTarget.value.id}`, {
            onSuccess: () => { showForm.value = false; },
        });
    } else {
        form.post('/admin/examinations/period/add', {
            onSuccess: () => { showForm.value = false; },
        });
    }
};
