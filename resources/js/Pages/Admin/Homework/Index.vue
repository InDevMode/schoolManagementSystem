        ...
        <div class="card overflow-hidden">
            <AppTable :columns="columns" :rows="works.data" :pagination="works" row-key="id">
                <template #cell-work_date="{ row }">
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ formatDate(row.work_date) }}</span>
                </template>
                <template #cell-submission_date="{ row }">
                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ formatDate(row.submission_date) }}</span>
                </template>
                <template #cell-description="{ row }">
                    <span class="line-clamp-2 text-sm text-gray-600 dark:text-gray-400">{{ stripHtml(row.description, 80) }}</span>
                </template>
            ...
            <div v-else-if="detailWork" class="space-y-5">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900 dark:text-white mt-1">{{ detailWork.class_name }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase font-semibold tracking-wide">Matière</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white mt-1">{{ detailWork.subject_name }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase font-semibold tracking-wide">Date travail</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white mt-1">{{ formatDate(detailWork.work_date) }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 uppercase font-semibold tracking-wide">Date remise</p>
                        <p class="text-sm font-medium text-gray-900 dark:text-white mt-1">{{ formatDate(detailWork.submission_date) }}</p>
                    </div>
                </div>
                ...
                <div v-if="detailWork.homeworks?.length">
                    <p class="text-xs text-gray-500 uppercase font-semibold tracking-wide mb-3">
                        Soumissions ({{ detailWork.homeworks.length }})
                    </p>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700 bg-white dark:bg-gray-800">
                                <tr v-for="hw in detailWork.homeworks" :key="hw.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                                    ...
                                    <td class="px-4 py-3 text-gray-500 dark:text-gray-400 text-xs">{{ formatDate(hw.created_at) }}</td>
                                    <td class="px-4 py-3">
...
const form = useForm({
    class_id: '', subject_id: '', work_date: '', submission_date: '', description: '',
});
...
// Ouvrir le modal détails en chargeant les données via Inertia visit
const openDetails = async (id: number) => {
    showDetails.value    = true;
    loadingDetails.value = true;
    detailWork.value     = null;
    try {
        const res  = await fetch(`/admin/practicalworks/homework/details-json/${id}`, {
