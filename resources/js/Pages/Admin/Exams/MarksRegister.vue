                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        <tr v-for="student in data.students" :key="student.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400">{{ student.class_name }}</td>
                            <td class="px-4 py-3">
...
interface MarksData {
    ...
    subjects?: unknown[];
    students?: { id: number; name: string; last_name: string; class_name: string }[];
}
...
const classOptions = computed(() =>
    (props.data.classes ?? []).map(c => ({ value: String(c.id), label: c.name }))
);

const applyFilters = () => {
    router.get('/admin/examinations/marks_register/list', filters.value, { preserveState: true, replace: true });
};
</script>
