<template>
    <div class="space-y-6">
        ...
        <!-- ── Header ──────────────────────────────────────────────────── -->
        <PageHeader title="Emploi du temps" :subtitle="student ? `${student.last_name} ${student.name}${classInfo ? ' · ' + classInfo.name : ''}` : 'Horaires de cours'" color="primary">
            <template #icon>
            ...
            <template v-if="student" #actions>
                ...
                <div class="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm">
                    ...
                    <div>
                        <p class="text-xs font-bold text-gray-900 dark:text-white leading-tight">{{ student.last_name }} {{ student.name }}</p>
            ...
            <div class="card overflow-hidden mb-4">
                <div class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800/60 flex items-center justify-between">
                    <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                        Vue hebdomadaire
                        <span v-if="subject" class="ml-2 normal-case font-normal text-primary-600 dark:text-primary-400">— {{ subject.name }}</span>
                    </h2>
                    ...
                    >
                        ...
                        <div class="flex items-center gap-3">
                            <div class="text-right">
                                <p class="text-sm font-bold text-primary-600 dark:text-primary-400">{{ day.start_time }}</p>
                                <p class="text-xs text-gray-400">→ {{ day.end_time }}</p>
                            </div>
                            <div v-if="day.room_number"
                                 class="flex items-center gap-1.5 text-xs font-medium text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2.5 py-1.5 rounded-xl border border-gray-200 dark:border-gray-600">
        ...
        <div v-else class="card p-14 text-center">
            ...
            <p class="text-gray-700 dark:text-gray-300 font-semibold">Aucun horaire disponible</p>
            <p class="text-gray-400 dark:text-gray-500 text-sm mt-1.5">L'emploi du temps n'a pas encore été configuré.</p>
        </div>
