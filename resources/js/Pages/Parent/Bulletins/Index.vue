<template>
    <div class="space-y-6">
        ...
        <PageHeader :title="`Bulletins de ${student.last_name} ${student.name}`" :subtitle="`${student.class_name ?? 'Aucune classe'}${bulletins.length ? ` · ${bulletins.length} bulletin${bulletins.length > 1 ? 's' : ''}` : ''}`" color="emerald">
            <template #icon>
