<template>
    <Teleport to="body">
        ...
        >
            <div v-if="modelValue" class="fixed inset-0 z-[10001] flex items-center justify-center p-4">
                ...
                >
                    <div v-if="modelValue" class="modal-with-pattern relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 overflow-hidden">
                        <!-- Illustration background subtile -->
                        <div class="pointer-events-none absolute inset-0 modal-bg-illustration" aria-hidden="true" />

                        <!-- Icône -->
                        <div :class="['relative z-10 w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4', iconBg]">
                        ...
                        <!-- Titre -->
                        <h3 class="relative z-10 text-base font-bold text-gray-900 dark:text-white text-center mb-2">{{ title }}</h3>

                        <!-- Message -->
                        <p class="relative z-10 text-sm text-gray-500 dark:text-gray-400 text-center mb-6">{{ message }}</p>
